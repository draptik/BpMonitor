...
2. Insert a new version section immediately after it, using the approved summary
   bullets grouped under `### Added`, `### Changed`, `### Fixed`, `### Security`,
   or `### Maintenance` as appropriate. Use today's date in ISO format
   (`YYYY-MM-DD`):
...
`release.yml` picks up the tag, reads its annotation as the top of the release
body, and appends categorized notes generated by `scripts/release-notes.sh` from
the commits since the previous tag (grouped by conventional-commit type, dependency
bumps collapsed into a `<details>` block). Because the per-PR list is generated
automatically, keep the tag annotation itself to a short human summary — don't
duplicate a per-PR bullet list there.
