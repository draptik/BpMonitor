...
- Dependency updates and CI/test reliability fixes. No user-facing changes.
...
- Settings: the Goal Range and Medications sections are now collapsible. The medications table's Start and End columns are now center-aligned, and its Edit action now renders as a button, matching Delete — which is now styled as a warning (red outline) and asks for confirmation, naming the medication, before it actually deletes anything. The add-medication form's fields are now labeled Required or Optional. Family Members table's Edit action likewise now matches Reset password's button style.
...
- Internal maintenance release: dependency updates and test reliability fixes. No user-facing changes.
...
- No user-facing changes in this release — internal tooling and documentation updates only (screenshot generation for the README)
...
- **Deployment:** Added `BpMonitor:SecureCookies` config key (default `false`) — set to `true` when running behind an HTTPS reverse proxy; leave unset for direct HTTP deployments
...
- Fixed a page shift that occurred when selecting a trends pill on mobile
...
- Refreshed navigation layout: always-visible top bar, sidebar icons, logout button moved to the top bar, and the app title links back to the home page
- Page headings now styled with the primary colour for better visual hierarchy
- Charts no longer show a redundant "Comments" y-axis label
...
[Unreleased]: https://github.com/draptik/BpMonitor/compare/v1.13.2...HEAD
