...
module LocalizedStrings =
  ...
  /// German values land here in the follow-up PR; every language routes through here
  /// so a third language is one new value plus one match arm.
  let forLanguage =
    function
    | English -> en
    | German -> { en with Language = German }
