...
let private makeRepo (initial: BloodPressureReading list) =
    ...
    { new IReadingRepository with
        ...
        member _.Update(r) = data <- data |> List.map (fun x -> if x.Id = r.Id then r else x) }

[<Fact>]
let ``pressing Esc invokes the onQuit callback`` () =
    let quitCalls = ResizeArray<unit>()
    use win = new ReadingsWindow(app, makeRepo [], Some (fun () -> quitCalls.Add(())), None, None, None)
...
let ``AddNew invokes the onAdd callback`` () =
    ...
    use win = new ReadingsWindow(app, makeRepo [], None, Some (fun () -> addCalls.Add(()); None), None, None)
    win.AddNew()
...
let ``when onAdd returns a reading it is added to the repository`` () =
    ...
    use win = new ReadingsWindow(app, repo, None, Some (fun () -> Some newReading), None, None)
    win.AddNew()
...
let ``EditSelected invokes the onEdit callback with the selected reading`` () =
    ...
    let repo = makeRepo [ reading 120 80 70; reading 130 85 72 ]
    use win = new ReadingsWindow(app, repo, None, None, Some (fun r -> editedReadings.Add(r); None), None)
    win.EditSelected()
...
let ``EditSelected with empty list does not invoke the onEdit callback`` () =
    let editedReadings = ResizeArray<BloodPressureReading>()
    use win = new ReadingsWindow(app, makeRepo [], None, None, Some (fun r -> editedReadings.Add(r); None), None)
    win.EditSelected()
...
let ``when onEdit returns an updated reading the repository is updated`` () =
    ...
    use win = new ReadingsWindow(app, repo, None, None, Some (fun _ -> Some updated), None)
    win.EditSelected()
...
let ``ShowChart invokes the onChart callback`` () =
    ...
    use win = new ReadingsWindow(app, makeRepo [], None, None, None, Some (fun _ -> chartCalls.Add(())))
    win.ShowChart()
...
let ``onChart callback receives all readings from the repository`` () =
    ...
    let repo = makeRepo [ reading 120 80 70; reading 130 85 72 ]
    use win = new ReadingsWindow(app, repo, None, None, None, Some (fun rs -> capturedReadings.Add(rs)))
    win.ShowChart()
